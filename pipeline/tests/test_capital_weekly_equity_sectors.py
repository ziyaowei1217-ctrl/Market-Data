...
from pipeline.capital_weekly.equity_sectors import fetch_equity_sectors, load_sector_universe
from pipeline.scripts import fetch_equity_sectors as fetch_cli
...
class EquitySectorFetchTests(unittest.TestCase):
    def test_fetcher_applies_as_of_cutoff_before_snapshot_calculation(self):
        with TemporaryDirectory() as directory:
            ...
            with patch(
                "pipeline.capital_weekly.equity_sectors.fetch_history",
    ...
    def legacy_investing_sector_appends_reported_ytd_baseline_after_session_filter(self):
        with TemporaryDirectory() as directory:
            ...
            with patch("pipeline.capital_weekly.equity_sectors.fetch_history",
    ...
    def test_cli_writes_strict_json_with_null_for_failed_numeric_values(self):
        ...
        with TemporaryDirectory() as directory:
            with patch("pipeline.scripts.fetch_equity_sectors.fetch_equity_sectors", return_value=(data, source_log)):
    ...
    def test_cli_rolls_back_whole_output_bundle_when_publish_fails(self):
        ...
        with TemporaryDirectory() as directory:
            ...
            with patch("pipeline.scripts.fetch_equity_sectors.fetch_equity_sectors",
                       return_value=(data, source_log)), \
                 patch("pipeline.scripts.fetch_equity_sectors.os.replace", side_effect=fail_staging_publish), \
