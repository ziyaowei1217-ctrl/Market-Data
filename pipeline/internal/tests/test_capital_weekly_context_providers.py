...
class ContextProviderTests(unittest.TestCase):
    def test_cftc_tff_provider_preserves_five_year_history_and_archive_provenance(self):
        header = (
        ...
        observations = {
            2022: ("2022-12-27", 100, 400),
            2023: ("2023-12-26", 200, 300),
            2024: ("2024-12-31", 300, 200),
            2025: ("2025-12-30", 400, 100),
            2026: ("2026-01-06", 500, 0),
        }
        archives = {}
        for year, (report_date, asset_net, leveraged_net) in observations.items():
            row = (
                f"S&P 500 Consolidated,{report_date},13874A,1000,"
                f"{asset_net + 500},500,{leveraged_net + 500},500\n"
            )
            archive_buffer = io.BytesIO()
            with zipfile.ZipFile(archive_buffer, "w") as archive:
                archive.writestr(f"tff_{year}.csv", header + row)
            archives[providers_module.CFTC_TFF_URL.format(year=year)] = (
                archive_buffer.getvalue()
            )

        requested_urls = []

        def fake_official_bytes(_session, url, _policy):
        ...
        with patch.object(
            providers_module,
            "_official_bytes",
            side_effect=fake_official_bytes,
        ):
            result = providers_module._cftc_tff_provider(
                object(),
                date(2025, 12, 29),
                date(2026, 1, 4),
                {"13874A": "SP500"},
                10,
                providers_module.load_commodity_http_policies()["cftc_tff"],
            )

        expected_urls = [
            providers_module.CFTC_TFF_URL.format(year=year)
            for year in range(2022, 2027)
        ]
        rows_by_metric = {row["metric_code"]: row for row in result.rows}
        with self.subTest("fetches five complete calendar years"):
            self.assertEqual(requested_urls, expected_urls)
        with self.subTest("calculates changes across archive boundaries"):
            self.assertEqual(
                rows_by_metric["SP500_asset_manager_net_change"]["value"],
                100,
            )
            self.assertEqual(
                rows_by_metric["SP500_leveraged_fund_net_change"]["value"],
                -100,
            )
        with self.subTest("emits both participant percentiles"):
            self.assertEqual(
                rows_by_metric["SP500_asset_manager_percentile"]["value"],
                1.0,
            )
            self.assertEqual(
                rows_by_metric["SP500_leveraged_fund_percentile"]["value"],
                0.25,
            )
        with self.subTest("attributes the selected row to its yearly archive"):
            expected_source_url = providers_module.CFTC_TFF_URL.format(year=2025)
            self.assertEqual(
                {
                    row["source_url"]
        ...
        with self.subTest("documents the public history and release semantics"):
            self.assertEqual(
                result.notes,
    ...
    def test_cftc_and_usda_transport_boundaries_preserve_policy_secret_and_parse_trace(self):
        ...
        for provider, parser, invoke, body, expected_calls in cftc_cases:
            with self.subTest(provider=provider):
                ...
                for _, kwargs in calls:
                    self.assertEqual(kwargs["policy"], policies[provider].policy)
                    self.assertEqual(kwargs["audit_secrets"], ())
    ...
    def test_legacy_eia_registry_routes_only_eia_commodities_rows(self):
        fields = [
            "provider",
            "commodity_code",
            "commodity_family",
        ...
        official = {
            ...
            "route": "natural-gas/stor/wkly",
            "frequency": "weekly",
            "facets": json.dumps({"series": "NW2_EPG0_SWO_R48_BCF"}),
            "metric_code": "eia_ng_storage_lower48",
                    ...
                    "data": [
                        {
                            "period": "2026-08-22",
                            "series": "WTESTUS1",
                            "series-description": "Weekly petroleum series",
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            with (data_dir / "capital_weekly_eia_series.csv").open(
                ...
                writer.writeheader()
                writer.writerows([legacy, official])
            session = TextSession(response)
            provider = build_default_providers(
                start=date(2026, 8, 17),
                end=date(2026, 8, 23),
                data_dir=data_dir,
        ...
        self.assertEqual(
            session.calls[0][1]["params"]["facets[series][]"],
            "WTESTUS1",
        )
    ...
    def test_unknown_eia_provider_is_not_routed_to_an_official_family(self):
        bad_natural = {
        ...
        audits = {row["provider"]: row for row in tables["source_log"]}
        self.assertEqual(audits["eia_natural_gas"]["status"], "FETCH_FAILED")
        self.assertIn("has no configured series", audits["eia_natural_gas"]["notes"])
        self.assertNotIn("misspelled_natural", audits["eia_natural_gas"]["notes"])
        self.assertEqual(audits["eia_refined_products"]["status"], "OK")
        self.assertEqual(len(tables["commodity_fundamentals"]), 3)
