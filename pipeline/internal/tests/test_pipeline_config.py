...
EXPECTED_SECTION_HASHES = {
    ...
    "context.eia_series": "4f4f171692463a7b8aad480df60dbcd32ef0abb05c390963864ed63fec133058",
...
class PipelineConfigTests(unittest.TestCase):
    def test_public_green_config_is_json_backed_and_watchlist_stays_empty(self):
        ...
        self.assertEqual(
            {row["report_family"] for row in cftc},
            {"tff", "disaggregated"},
        )
    ...
    def test_eia_config_covers_independent_physical_fundamental_families(self):
        ...
        official_rows = [
            row
            for row in rows
            if row["provider"] in {"eia_natural_gas", "eia_refined_products"}
        ]
        legacy_rows = [
            row for row in rows if row["provider"] == "eia_commodities"
        ]
        self.assertEqual(
            legacy_rows,
            [
        ...
        self.assertTrue(all(required_fields <= set(row) for row in official_rows))
        by_provider = {}
    ...
    def test_all_five_pipelines_use_json_by_default(self):
        ...
        self.assertEqual(len(load_macro_asset_universe()), 81)
        providers = build_default_providers(
            start=date(2026, 8, 3),
            end=date(2026, 8, 9),
            environ={},
    ...
    def test_official_gold_row_configures_its_daily_correlation_proxy(self):
        macro = load_config_rows("macro")
        gold = next(row for row in macro if row["series_code"] == "COMEX_GOLD")

        self.assertEqual(len(macro), 81)
        self.assertEqual(
            (gold["provider"], gold["provider_symbol"], gold["frequency"]),
            ("world_bank_pink_sheet", "Gold", "monthly"),
        )
        self.assertEqual(
            {
                key: gold.get(key)
            ...
            {
                "correlation_proxy_provider": "yahoo_chart",
                "correlation_proxy_symbol": "GC=F",
                "correlation_proxy_source": (
                    "Yahoo Finance chart API (public vendor proxy)"
                ),
                "correlation_proxy_source_url": (
                    "https://query2.finance.yahoo.com/v8/finance/chart/GC%3DF"
                ),
                "correlation_proxy_frequency": "daily",
            },
        )
        self.assertEqual(
            [
                row["series_code"]
                for row in macro
                if row.get("correlation_proxy_provider")
            ],
            ["COMEX_GOLD"],
        )
