...
VERSION_4_RELEASE_DATASETS = tuple(
    dataset
    for dataset in VERSION_5_RELEASE_DATASETS
    if not (
...
def release_datasets_for_contract(
    ...
    if dataset_contract_version == PUBLIC_GREEN_DATASET_CONTRACT_VERSION:
        return VERSION_5_RELEASE_DATASETS
...
def _validate_row(
    ...
    is_current_macro_source_log = (
        ...
        in {
            ...
            PUBLIC_GREEN_DATASET_CONTRACT_VERSION,
            DATASET_CONTRACT_VERSION,
        }
    ...
    for column in spec.timestamp_columns:
        ...
        if not raw_timestamp and spec.pipeline == "macro_assets":
            continue
        try:
...
def _validate_configured_commodity_coverage(
    ...
    eia_config = _configured_rows(
        ...
        include=lambda row: bool(row.get("commodity_code")),
    )
...
def validate_output_bundle(output_root: Path) -> dict:
    ...
    is_unversioned = "dataset_contract_version" not in release
    ...
    has_capabilities = "capabilities" in release
    ...
    for name in OUTPUT_BUSINESS_FILES:
        ...
        if is_unversioned:
            document_contract_matches = (
                "dataset_contract_version" not in document
            )
            expected_tables = set(
                UNVERSIONED_LEGACY_OUTPUT_TABLES[expected_pipeline]
            )
            if expected_pipeline == "context" and has_capabilities:
                expected_tables.add("capability_audit")
        else:
            document_contract_matches = (
                document.get("dataset_contract_version") == contract_version
            )
            expected_tables = {
                table_name
                for table_name, _filename in output_tables[expected_pipeline][1]
            }
        if not is_unversioned and expected_pipeline == "context":
            expected_tables.add("capability_audit")
        if (
            document.get("schema_version") != OUTPUT_SCHEMA_VERSION
            or not document_contract_matches
    ...
    expected_pipeline_names = (
        set(UNVERSIONED_LEGACY_OUTPUT_TABLES)
        if is_unversioned
        else set(output_tables)
    )
    if set(pipeline_entries) != expected_pipeline_names:
    ...
    for name, entry in pipeline_entries.items():
        ...
        if is_unversioned:
            expected_rows = {
                table_name: len(rows)
                for table_name, rows in document["tables"].items()
            }
        else:
            expected_rows = {
            ...
            if name == "context":
                expected_rows["capability_audit"] = len(
                    document["tables"]["capability_audit"]
                )
