...
market data/
├── pipeline/
│   ├── capital_weekly/   # 采集、计算、清洗、校验与发布实现
│   ├── scripts/          # 内部兼容和辅助命令
│   ├── tests/            # 确定性测试与夹具
│   ├── docs/             # 设计、计划和结构说明
...
| 模块 | 职责 |
| --- | --- |
| `capital_weekly/equity_indices.py` | 多提供方全球股指历史、时间截断和收益快照。 |
| `capital_weekly/equity_sectors.py` | A 股、港股、美股行业数据和抓取审计。 |
| `capital_weekly/gics_sectors.py` | 美国 GICS 行业 ETF 代理。 |
| `capital_weekly/macro_assets.py` | 固收、政策利率、货币市场、外汇、商品和登记派生序列。 |
| `capital_weekly/sector_divergence.py` | 行业排名、广度和分化。 |
| `capital_weekly/macro_divergence.py` | 宏观分组排名和分化。 |
| `capital_weekly/history.py` | `as_of_date` 历史截断。 |
| `capital_weekly/returns.py` | 日、周、MTD、YTD 变化。 |

### 周度背景模块

`capital_weekly/weekly_context.py` 协调提供方并保留空表标准结构。
`capital_weekly/context/` 将事件、经济发布、金融条件、波动率、市场内部、
...
`capital_weekly/weekly_release.py` 是发布契约的单一事实来源，负责：
...
`sort_order` 等类型。`pipeline/common.py` 保留显式临时 CSV 路径兼容，
