...
def main(argv=None, *, release_runner=run_weekly_release) -> int:
    parser = argparse.ArgumentParser(
        description="Atomically refresh and publish a complete Capital Weekly snapshot."
    )
    parser.add_argument(
        "--as-of-date",
        type=date.fromisoformat,
        default=None,
        help="Override the release window end date (must be a Sunday).",
    )
    parser.add_argument(
        "--status-file",
        type=Path,
