...
from capital_weekly.gics_sectors import fetch_gics_sectors


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch GICS sector data from free public APIs."
    )
    parser.add_argument(
        "--universe",
        default="data/capital_weekly_gics_sectors.csv",
        help="CSV universe config path.",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Output directory. Defaults to outputs/capital_weekly_gics_sectors_python_<timestamp>.",
    )
    parser.add_argument(
        "--no-raw-cache",
        action="store_true",
    ...
    output_dir = Path(
        args.output_dir
        or f"outputs/capital_weekly_gics_sectors_python_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    (output_dir / "gics_sectors_snapshot.json").write_text(
