...
from capital_weekly.macro_assets import (
    _atomic_write_bytes,
    _parse_fred_csv,
...
class MacroAssetUniverseTests(unittest.TestCase):
    ...
    def test_strict_records_preserves_change_precision_below_1e_12(self):
        from scripts.fetch_macro_assets import strict_records
    ...
    def test_boj_newest_malformed_candidate_fails_instead_of_carrying_old_rate(self):
        ...
        with patch("capital_weekly.macro_assets._parse_boj_policy_summary_pdf") as parser:
            parser.side_effect = [
                {"date": date(2025, 12, 19), "value": 0.75},
    ...
    def test_fred_dispatch_bounds_request_dates(self):
        ...
        with patch("capital_weekly.macro_assets.date") as mocked_date:
    ...
    def test_hkma_false_total_count_hits_page_guard(self):
        ...
        session = unittest.mock.Mock(_macro_attempt_trace=[], _macro_raw_parts=[])
        session.get.return_value = response
        with patch("capital_weekly.macro_assets.MAX_PROVIDER_PAGES", 3):
    ...
    def test_blocked_real_provider_caches_raw_and_records_exact_provenance(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            session.get.return_value = response
            with patch("capital_weekly.macro_assets._session", return_value=session):
    ...
    def test_hibor_fetches_bounded_anchor_windows_with_all_snapshot_bases(self):
        ...
        snapshot = __import__("capital_weekly.returns", fromlist=["calculate_macro_snapshot"]).calculate_macro_snapshot(history, "bp")
    ...
    def test_lpr_fixing_is_carried_forward_on_business_days_through_as_of_date(self):
        ...
        with patch("capital_weekly.macro_assets.date") as mocked_date, patch(
            "capital_weekly.macro_assets._post", return_value=response
        ), patch(
            "capital_weekly.macro_assets._parse_lpr_xlsx",
            return_value=[
    ...
    def test_failure_audit_uses_attempt_trace_instead_of_generic_source_url(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("capital_weekly.macro_assets._fetch_config_history", side_effect=fail_with_trace):
    ...
    def test_raw_response_is_cached_when_parsing_fails(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("capital_weekly.macro_assets._fetch_config_history", side_effect=parse_failure):
    ...
    def test_atomic_raw_cache_stages_in_destination_and_replaces_without_temp_leftover(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("capital_weekly.macro_assets.os.replace", side_effect=recording_replace):
    ...
    def test_raw_cache_failure_is_a_separate_audit_failure(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("capital_weekly.macro_assets._fetch_config_history", return_value=(history, b"raw", "https://expanded.test")), patch(
                "capital_weekly.macro_assets._atomic_write_bytes", side_effect=OSError("disk full")
            ):
    ...
    def test_partial_fetch_failure_still_emits_one_detail_and_source_row_per_config(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("capital_weekly.macro_assets._fetch_config_history", side_effect=fake_fetch):
    ...
    def test_cli_writes_strict_json_with_null_for_failed_numeric_values(self):
        ...
        with tempfile.TemporaryDirectory() as directory, patch(
            "scripts.fetch_macro_assets.fetch_macro_assets",
    ...
    def test_cli_rolls_back_whole_output_bundle_when_publish_fails(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                ...
                "scripts.fetch_macro_assets.os.replace", side_effect=fail_staging_publish
            ), patch.object(
